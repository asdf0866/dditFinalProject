	...
	<div class="content w-100">
		...
		<div class="subIndex">
			<div class="subHeader my-2">
				<span class="mx-2 fw-bold fs-4">1:1 문의</span>
			</div>
			<div class="subContent">
				<div class="myResume">
					<div class="infoTitle">
						<span class='titleText'>내 문의</span>
					</div>
				</div>
				...
				<c:choose>
					<c:when test="${QnaList == '' or QnaList == null }">
						<ul class="infoContent">
							<li>작성한 1:1문의 글이 없습니다.</li>
						</ul>
						</c:when>
					<c:otherwise>
						<c:forEach items="${QnaList }" var="qna" varStatus="stat">
							<ul class="infoContent">
								...
								<li class="col-5 recTitle">
									<input type="hidden" value="${qna.boardNo }" name="boardNo"/>
									<c:set var="qnaTitle" value="${qna.title }" />
									<c:if test="${qnaTitle.length() <= 20 }">${qnaTitle }</c:if>
									<c:if test="${qnaTitle.length() > 20 }">${qnaTitle.substring(0, 20) }...</c:if>
								</li>
								<li class="col-2 text-center">${qna.hit }</li>
								<li class="col-2 text-center">
									<fmt:parseDate value="${qna.wrtDt }" pattern="yyyy-MM-dd" var="wrtDt"/>
									<fmt:formatDate value="${wrtDt }" pattern="yyyy-MM-dd"/>
								</li>
								<li class="col-2 text-center">
									<button type="button" class="content_item_btn detailBtn" value="${qna.boardNo }" style="float:none;">상세보기</button>
								</li>
							</ul>
						</c:forEach>
					</c:otherwise>
				...
				<form id="applyPageForm" action="" method="get">
					<input type="hidden" name="menu" value="myQna">
